Shader "Custom/HeatQuad" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float2 densityIndex = IN.worldPos.rg / _CellWidth;
			
			half densityValue = tex2D(_DensityTex, densityIndex).r;
			fixed4 densityColor = tex2D(_HeatMapScaleTex, fixed2(0.0, densityValue));
